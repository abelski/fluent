...
export default function SettingsPage() {
  ...
  async function handlePhrasesSave() {
    ...
    try {
      await Promise.all([
        updatePhrasesSettings(phrasesSettings),
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        {activeTab === 'vocabulary' ? (
          ...
          <div className="bg-white border border-gray-900 rounded-2xl p-6 flex flex-col gap-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  max={30}
                  step={1}
                  value={phrasesSettings.phrases_per_session}
                  onChange={(e) => setPhrasesSettings((prev) => ({ ...prev, phrases_per_session: parseInt(e.target.value, 10) }))}
                  data-testid="phrases-session-size-slider"
                  className="flex-1 accent-emerald-600"
                />
                <span className="w-8 text-center font-semibold text-gray-900 tabular-nums">
                  {phrasesSettings.phrases_per_session}
                </span>
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">
                {tr.settings.phrasesRatioLabel}
              </label>
              <p className="text-xs text-gray-400 mb-3">{tr.settings.phrasesRatioHint}</p>
              {(() => {
                const newPct = Math.round(phrasesSettings.new_phrases_ratio * 100);
                const reviewPct = 100 - newPct;
                const newCount = Math.round(phrasesSettings.phrases_per_session * phrasesSettings.new_phrases_ratio);
                const reviewCount = phrasesSettings.phrases_per_session - newCount;
                return (
                  <>
                    <div className="flex h-8 rounded-lg overflow-hidden mb-3 text-xs font-medium">
                      <div
                        className="flex items-center justify-center text-xs font-medium text-white bg-emerald-600 transition-all duration-150"
                        style={{ width: `${newPct}%` }}
                      >
                        {newPct > 15 && `${tr.settings.ratioNewLabel} ${newPct}%`}
                      </div>
                      <div
                        className="flex items-center justify-center text-xs font-medium text-gray-700 bg-gray-100 transition-all duration-150"
                        style={{ width: `${reviewPct}%` }}
                      >
                        {reviewPct > 15 && `${tr.settings.ratioReviewLabel} ${reviewPct}%`}
                      </div>
                    </div>
                    <input
                      type="range"
                      ...
                      step={5}
                      value={newPct}
                      onChange={(e) => setPhrasesSettings((prev) => ({ ...prev, new_phrases_ratio: parseInt(e.target.value, 10) / 100 }))}
                      data-testid="phrases-ratio-slider"
